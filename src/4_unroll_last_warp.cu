...
__global__ void reduction_unroll_last_warp_kernel(float *d_output, const float *d_input, const int numElements) {
...
void reduction_unroll_last_warp(float *d_output, const float *d_input, const int numElements) {
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSM, reduction_unroll_last_warp_kernel, BLOCK_SIZE, 0);
    ...
    reduction_unroll_last_warp_kernel<<<gridSize, blockSize, sharedMemSize, 0>>>(d_output, d_input, numElements);
    reduction_unroll_last_warp_kernel<<<1, blockSize, sharedMemSize, 0>>>(d_output, d_output, gridSize);
}
